...
//        horizontalAlignment = Alignment.CenterHorizontally
        ...
        EmailField(
            uiState.email, loginViewModel::onEmailChange, Modifier.textFieldModifier()
        )
        PasswordField(
            uiState.password, loginViewModel::onPasswordChange, Modifier.textFieldModifier()
        )
        Row(
            ...
            Checkbox(
                checked = uiState.remember, onCheckedChange = loginViewModel::onRememberChange
            )
        }

        val context = LocalContext.current
        BasicButton(
            AppText.sign_in, Modifier.buttonModifier()
        ) {
            loginViewModel.onSignInClick(onClick = onClickLogin, context)
        }
        BasicButton(
            text = AppText.create_account, modifier = Modifier.buttonModifier()
        ) {
