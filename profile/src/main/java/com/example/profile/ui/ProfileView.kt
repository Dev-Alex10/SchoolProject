...
@Composable
fun ProfileView(
    modifier: Modifier = Modifier, profileViewModel: ProfileViewModel = hiltViewModel()
) {
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        Column {
            ...
            LazyColumn(state = scrollState,
                modifier = Modifier.fillMaxWidth().padding(start = 25.dp).background(Color.White)
                    .clickable { }) {
                items(list) { item ->
                    Row {
...
private fun ProfileCardCollapsingToolbar(scrollOffset: Float) {
    ...
    Row(
        Modifier.fillMaxWidth().padding(top = 30.dp, start = 25.dp).clickable { },
    ) {
        AsyncImage(
            model = "https://images.unsplash.com/photo-1671275285749-1d89bf2504f7?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=764&q=80",
            contentDescription = "Profile Pic",
        ...
        Column {
            ...
            Text(
                text = Firebase.auth.currentUser?.email!!, maxLines = linesCount
            )
        }
    }
}
